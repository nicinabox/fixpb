...
const getUrlParts = ({ url, username, filename, mediaId, key }) => {
  return {
    username,
    filename,
    url,
    key: decodeURIComponent(key),
    mediaId: mediaId || getMediaId(filename),
  }
}
...
const matchDirect = (pbUrl) => {
  ...
  return getUrlParts({ url: pbUrl, key, album, username, filename })
}
...
const matchMediaHtml = (pbUrl) => {
  ...
  return resolveDirectUrl({ url: toMediaSubdomain(pbUrl) })
    .then((url) => {
      ...
      return getUrlParts({ url, username, filename, key })
    })
}
...
const matchMediaGallery = (pbUrl) => {
  ...
  return resolveDirectUrl({ username, mediaId })
    .then((url) => {
      let [, key] = url.match(patterns.directHtml)
      return getUrlParts({ url: toHttps(url), username, filename, mediaId, key })
    })
}
...
const matchDirectHtml = (pbUrl) => {
  ...
  let [,key, album, username, filename] = parts
  return getUrlParts({ key, url: toHttps(pbUrl), username, filename })
}

const parseUrl = (pbUrl) => {
  const testUrl = test(pbUrl)

  if (!patterns.photobucket.test(pbUrl)) return Promise.reject('Enter a photobucket url')
  if (patterns.library.test(pbUrl)) return Promise.reject('Sorry, we can\'t parse library urls')
